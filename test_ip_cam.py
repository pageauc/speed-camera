...
class WebcamVideoStream:
    def __init__(self, CAM_SRC=WEBCAM_SRC, CAM_WIDTH=WEBCAM_WIDTH,
        ...
        from the stream
        """
        self.stream=open('output.mjpg','rb')
        bytes=''
        self.stream = CAM_SRC
...
class IPCamera(object):
	...
	def __init__(self,camURL, cameraFunction, dlibDetection, fpsTweak):
		...
		self.FPScount = 0
		self.motion = False # Used for alerts and transition between system states i.e from motion detection to face detection
	...
	def read_jpg(self):
		...
		the client. It is necessary to make the image smaller to
